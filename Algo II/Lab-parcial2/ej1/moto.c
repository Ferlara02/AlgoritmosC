...
void reparar_moto(moto_t moto) {
    /*{------- RUEDA DELANTERA -------}*/
    //si la rueda delantera NO esta pinchada pero presion != optima, inflamos
    if(!moto->rueda_delantera->pinchada && moto->rueda_delantera->presion != PRESION_OPTIMA){
        moto->rueda_delantera->presion = PRESION_OPTIMA;
    }
    //si esta pinchada, la destruimos, creamos una nueva y la inflamos 
    else if(moto->rueda_delantera->pinchada) {
    ...
    /*{------- RUEDA TRASERA -------}*/    
    //si la rueda trasera NO esta pinchada pero presion != optima, inflamos
    if(!moto->rueda_trasera->pinchada && moto->rueda_trasera->presion != PRESION_OPTIMA) {
        moto->rueda_trasera->presion = PRESION_OPTIMA;
    } 
    //si esta pinchada, la destruimos, creamos una nueva y la inflamos
    else if (moto->rueda_trasera->pinchada) {
